...
is-bijection' f = Σ (λ inverse → (inverse ∘ f ∼ id) × (f ∘ inverse ∼ id))

_≅'_ : Type → Type → Type
A ≅' B = Σ (λ (bijection : A → B) → is-bijection' bijection)
```
...
well-ordering-principle P d (suc n) p = well-ordering-principle-suc P d n p (d 0) IH
  where
    IH : minimal-element (λ m → P (suc m))
    IH = well-ordering-principle _ (d ∘ suc) n p
```
...
is-zero-well-ordering-principle P d zero    p p0 = refl 0
is-zero-well-ordering-principle P d (suc m) pm   =
  is-zero-well-ordering-principle-suc P d m pm (d 0) lemma
  where
    lemma : minimal-element (λ n → P (suc n))
    lemma = well-ordering-principle (λ m → P (suc m)) (d ∘ suc) m pm
```
